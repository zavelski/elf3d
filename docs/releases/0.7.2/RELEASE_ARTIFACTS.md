# Elf3D 0.7.2 Release Artifacts

Purpose: Define and record expected release assets for Elf3D 0.7.2.
...
validation. No 0.7.2 assets have been published on GitHub.
...
| Asset | Size | SHA-256 |
| --- | ---: | --- |
| `elf3d-viewer-0.7.2-windows-x64.zip` | 1,503,499 bytes | `a20d8ad1483c9237a9536b29e6c327398a74a2e7a57cef07fd42da535d1c96c4` |
| `SHA256SUMS.txt` | 102 bytes | Contains the matching viewer ZIP checksum. |

Two local package runs against the same staged Release binaries produced the
same ZIP SHA-256. This confirms deterministic package metadata for a fixed file
set; it does not claim compiler-level binary reproducibility across machines.
...
`out/release/verify-0.7.2-final`; the packaged `elf3d_viewer.exe` stayed alive
