# Elf3D 0.7.2 Release Snapshot Project State

Purpose: Snapshot the Elf3D 0.7.2 project state for local release review.
...
Elf3D 0.7.2 includes:

- version API returning `0.7.2`;
...
- OpenGL 4.1 off-screen viewport rendering with linear alpha blending before
  display transfer encoding and separate alpha-channel source-over blending;
- real hidden-context OpenGL smoke coverage for GLSL compilation and one
  transparent pixel result, including resolved texture alpha;
...
- deterministic ZIP entry order/timestamps for fixed staged package inputs;
- release workflow version derivation from `CMakeLists.txt`.
