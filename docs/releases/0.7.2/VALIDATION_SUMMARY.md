# Elf3D 0.7.2 Validation Summary

Purpose: Record local validation performed for the Elf3D 0.7.2 release source.
...
`tests/fixtures/textured_pbr.gltf`, `third_party`, `docs/releases/0.7.2`
...
.\scripts\package_release.ps1 -OutputDir out/release
Expand-Archive -LiteralPath out/release/elf3d-viewer-0.7.2-windows-x64.zip -DestinationPath out/release/verify-0.7.2-final
.\scripts\package_release.ps1 -OutputDir out/release-determinism-0.7.2-a
.\scripts\package_release.ps1 -OutputDir out/release-determinism-0.7.2-b
```
...
| Debug tests | Passed | 18/18 passed, including real OpenGL shader/pixel smoke with resolved alpha coverage. |
...
| Release tests | Passed | 18/18 passed, including real OpenGL shader/pixel smoke with resolved alpha coverage. |
...
| Release package | Passed | Generated the 0.7.2 viewer ZIP and checksum file using the default CMake build output and package-version derivation from `CMakeLists.txt`. |
...
| Deterministic package metadata | Passed | Two local package runs against the same staged Release binaries produced matching ZIP SHA-256 values. |
| Viewer interaction validation | Not run | No visible/manual viewer interaction pass was performed after the 0.7.2 fixes. |
...
```text
a20d8ad1483c9237a9536b29e6c327398a74a2e7a57cef07fd42da535d1c96c4  elf3d-viewer-0.7.2-windows-x64.zip
```

ZIP size: 1,503,499 bytes.
