...
int main() {
    ...
    constexpr elf3d::Bounds3 default_bounds{{-1.0F, -1.0F, -1.0F},
                                            {1.0F, 1.0F, 1.0F},
                                            true};
    ...
    const elf3d::Bounds3 centered{{-1.0F, -1.0F, -1.0F}, {1.0F, 1.0F, 1.0F}, true};
    const elf3d::Bounds3 positive{{0.25F, -1.0F, -1.0F}, {1.0F, 1.0F, 1.0F}, true};
    const elf3d::Bounds3 negative{{-1.0F, -1.0F, -1.0F}, {-0.25F, 1.0F, 1.0F}, true};
    ...
    const elf3d::Bounds3 clipped = elf3d::clipping::clipped_bounds(plane_filter.value(), centered);
    if (!clipped.is_valid || !nearly_equal(clipped.minimum.x, 0.0F) ||
        !nearly_equal(clipped.maximum.x, 1.0F)) {
        return 13;
    }
    const elf3d::Bounds3 union_bounds = elf3d::clipping::clipped_bounds(boxes_filter.value(),
                                                                        centered);
    if (!union_bounds.is_valid || !nearly_equal(union_bounds.minimum.x, -1.0F) ||
        !nearly_equal(union_bounds.maximum.x, 1.0F)) {
        return 14;
    }
    const elf3d::Bounds3 empty =
    ...
    const elf3d::math::Matrix4 translated =
    ...
    if (!transformed.is_valid || !nearly_equal(transformed.minimum.x, 3.0F) ||
