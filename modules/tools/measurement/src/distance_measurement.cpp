...
Result<DistanceMeasurementController::ResolvedAnchor> DistanceMeasurementController::resolve_anchor(
    const scene::Storage &scene, const scene::VisibilityFilter &visibility,
    const clipping::ClippingFilter &clipping_filter, const MeasurementAnchor &anchor) const
    noexcept {
    ...
    const math::Vector3 a = math::to_vector(mesh.vertices[i0].position);
    const math::Vector3 b = math::to_vector(mesh.vertices[i1].position);
    const math::Vector3 c = math::to_vector(mesh.vertices[i2].position);
    const math::Vector3 local_position = a * anchor.barycentric_coordinates.x +
                                         b * anchor.barycentric_coordinates.y +
                                         c * anchor.barycentric_coordinates.z;
    const math::Vector3 local_normal = glm::cross(b - a, c - a);
    const float local_normal_length = glm::length(local_normal);
    ...
    point.world_normal = math::to_float3(world_normal);
