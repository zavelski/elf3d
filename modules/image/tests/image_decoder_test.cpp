...
constexpr std::array<std::uint8_t, 71> gray_png{{
    ...
    0xf8,0x00,0x00,0x00,0x0e,0x49,0x44,0x41,0x54,0x78,0x9c,0x63,0x60,0x70,0x60,0x68,
    0xf8,0x0f,0x00,0x03,0x05,0x01,0xc0,0x4e,0x33,0x5b,0xe9,0x00,0x00,0x00,0x00,0x49,
...
std::vector<std::byte> base64(std::string_view source) {
    ...
    for (std::size_t index = 0; index < source.size(); index += 4) {
        const std::uint32_t value =
            ...
            (static_cast<std::uint32_t>(source[index + 2] == '=' ? 0 : base64_value(source[index + 2])) << 6U) |
            static_cast<std::uint32_t>(source[index + 3] == '=' ? 0 : base64_value(source[index + 3]));
        result.push_back(static_cast<std::byte>((value >> 16U) & 0xffU));
        if (source[index + 2] != '=') result.push_back(static_cast<std::byte>((value >> 8U) & 0xffU));
        if (source[index + 3] != '=') result.push_back(static_cast<std::byte>(value & 0xffU));
    }
    return result;
}
...
int main() {
    ...
    if (!rgb || rgb.value().width != 2 || rgb.value().height != 2 ||
        ...
        pixel(rgb.value(), 1) != 0 || pixel(rgb.value(), 4) != 0 ||
        pixel(rgb.value(), 5) != 255 || pixel(rgb.value(), 8) != 0 ||
        pixel(rgb.value(), 10) != 255) {
        return 1;
    }
