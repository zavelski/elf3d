...
class FakeDevice final : public elf3d::graphics::Device {
  public:
    ...
    read_picking_depths(elf3d::graphics::PickingTarget& target) noexcept override {
        ...
        return std::vector<float>(static_cast<std::size_t>(extent.width) *
...
int elf3d_renderer_test() {
    ...
    const auto renderer = elf3d::renderer::Renderer::create(std::move(owned_device), engine_token);
    if (!renderer) {
        return 3;
    }
    ...
    device->picking_pixel = elf3d::graphics::PickingPixel{2U, 1U, 0U, 0.5F};
    ...
    const auto gpu_pick = renderer.value()->gpu_pick(
        scene, camera.value(), picking_target, {319.5F, 179.5F}, picking_target.extent_value,
        {319.5F, 179.5F}, visibility, elf3d::clipping::disabled_filter());
    ...
    const elf3d::Extent2D anchor_viewport_extent{640, 360};
    device->picking_depths.assign(16U, 1.0F);
    device->picking_depths[static_cast<std::size_t>(1U * 4U + 1U)] = 0.25F;
    device->picking_depths[static_cast<std::size_t>(1U * 4U + 3U)] = 0.75F;
    const auto focus_anchor = renderer.value()->gpu_focus_depth_anchor(
        scene, camera.value(), anchor_target, anchor_viewport_extent, visibility,
        elf3d::clipping::disabled_filter());
    const auto anchor_list =
        elf3d::renderer::build_render_list(scene, camera.value(), anchor_viewport_extent,
    ...
    const elf3d::Float2 expected_screen{static_cast<float>(anchor_viewport_extent.width) * 0.5F,
                                        static_cast<float>(anchor_viewport_extent.height) * 0.5F};
    const elf3d::Result<elf3d::Float3> expected_anchor = elf3d::math::unproject_viewport_point(
        anchor_list.value().view_matrix, anchor_list.value().projection_matrix,
        anchor_viewport_extent, expected_screen, expected_depth);
    if (!expected_anchor || !focus_anchor || !focus_anchor.value().world_position.has_value() ||
        focus_anchor.value().draw_calls != 2 || focus_anchor.value().pixels_read != 16 ||
        !nearly_equal(focus_anchor.value().world_position.value(), expected_anchor.value(),
                      0.0001F)) {
        return 46;
    }
    const elf3d::Result<elf3d::math::ViewportProjection> projected_anchor =
        elf3d::math::project_world_to_viewport_point(
            anchor_list.value().view_matrix, anchor_list.value().projection_matrix,
            anchor_viewport_extent, focus_anchor.value().world_position.value());
    if (!projected_anchor ||
    ...
    if (!renderer.value()->render(second_scene, second_camera.value(), target, {}, {}) ||
        device->upload_count != 2) {
        return 6;
    }
    if (!renderer.value()->render(scene, camera.value(), target, {}, {}) ||
        device->upload_count != 3 || device->texture_upload_count != 6) {
        return 6;
    }
