...
make_gpu_pick_hit(const RenderList &list, const RenderItem &item, graphics::PickingPixel pixel,
    ...
    const math::Vector4 world = inverse_view_projection * math::Vector4{ndc_x, ndc_y, ndc_z, 1.0F};
    ...
    try {
            ...
            for (std::uint32_t primitive_index = 0;
                ...
                list.items.push_back(
                    RenderItem{record->id, primitive.mesh, primitive.material, primitive_index,
                               model.value(), normals.value(), orientation_reversed,
        ...
        std::stable_sort(list.items.begin(), list.items.end(),
                         [](const RenderItem &left, const RenderItem &right) {
                             ...
                             return left_blended &&
                                    left.camera_distance_squared > right.camera_distance_squared;
                         });
        return list;
    } catch (...) {
...
Result<RenderStatistics> Renderer::render(const scene::Storage &scene_storage, EntityId camera,
    ...
    for (const RenderItem &item : list.items) {
        ...
        graphics::Texture2D *occlusion_texture = nullptr;
        graphics::Texture2D *emissive_texture = nullptr;
        ...
        if (material.occlusion_texture.is_valid()) {
            ...
            occlusion_texture = texture_result.value();
            ++statistics.texture_bindings;
        }
        if (material.emissive_texture.is_valid()) {
            ...
            emissive_texture = texture_result.value();
            ++statistics.texture_bindings;
        }
        ...
        draw.emissive_texture = emissive_texture;
