...
[[nodiscard]] Bounds3 triangle_bounds(Float3 a, Float3 b, Float3 c) noexcept {
    ...
    return to_bounds3(bounds.value());
}
...
[[nodiscard]] Bounds3 transform_bounds(Bounds3 local_bounds, const math::Matrix4 &world) noexcept {
    ...
    return to_bounds3(result.value());
}
...
class PickingService::Impl final {
  public:
    ...
    refine_candidate(const scene::Storage &scene, EntityId camera, Extent2D extent,
        ...
        const ModelPrimitiveBinding &primitive =
            record.model->primitives[candidate.primitive_index];
        if (primitive.mesh != candidate.mesh) {
            return std::optional<PickHit>{};
        }
        ...
        const Result<const assets::MeshAsset *> mesh_result = scene.assets().mesh(primitive.mesh);
        if (!mesh_result) {
            return mesh_result.error();
        }
        const Result<const assets::MaterialAsset *> material_result =
            scene.assets().material(primitive.material);
        if (!material_result) {
        ...
        std::optional<TriangleHit> triangle_hit =
            intersect_ray_triangle(local_ray.value(), mesh.vertices[i0].position,
                                   mesh.vertices[i1].position, mesh.vertices[i2].position,
                                   cull_back_face);
        ...
        for (const std::optional<scene::EntityRecord> &record : scene.entities()) {
            ...
            for (std::uint32_t primitive_index = 0;
                ...
                const Result<const assets::MeshAsset *> mesh_result =
                    scene.assets().mesh(primitive.mesh);
                if (!mesh_result) {
                    return mesh_result.error();
                }
                const Result<const assets::MaterialAsset *> material_result =
                ...
                const Result<const MeshAcceleration *> acceleration_result =
                    acceleration(scene.id(), primitive.mesh, mesh);
  ...
  private:
    ...
    [[nodiscard]] std::uint32_t build_node(MeshAcceleration &acceleration, std::uint32_t first,
        ...
        std::optional<BoundsD> node_bounds;
        std::optional<BoundsD> centroid_bounds;
        ...
        BvhNode node;
        ELF3D_ASSERT(node_bounds.has_value() && centroid_bounds.has_value());
        node.bounds = to_bounds3(node_bounds.value());
        ...
        const int axis = longest_axis(to_bounds3(centroid_bounds.value()));
