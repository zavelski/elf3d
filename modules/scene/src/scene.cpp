...
Result<SceneHierarchySnapshot> Scene::hierarchy_snapshot() const {
    ...
    try {
        ...
        while (!stack.empty()) {
            ...
            stack.pop_back();
            const Result<const scene::EntityRecord *> record =
                impl_->storage.entity(frame.entity);
            if (!record) {
                return record.error();
            }
            ...
            item.parent = record.value()->parent.value_or(EntityId{});
        ...
        return Error{ErrorCode::unexpected_exception,
                     "Hierarchy snapshot creation failed while allocating storage"};
    }
}
...
Bounds3 Scene::world_bounds() const noexcept {
    return impl_ != nullptr ? impl_->storage.world_bounds() : Bounds3{};
}

Bounds3 Scene::visible_bounds() const noexcept {
    if (impl_ == nullptr) {
        return Bounds3{};
    }
