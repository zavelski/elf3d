...
[[nodiscard]] int populate_public_scene(PublicSceneFixture& fixture) {
    ...
    const auto model = fixture.scene->create_model(mesh.value(), material.value());
    const auto camera =
        fixture.scene->create_perspective_camera(elf3d::PerspectiveCameraDescription{});
    if (!model || !camera ||
    ...
    elf3d::PerspectiveCameraDescription camera_description =
        fixture.scene->perspective_camera(fixture.camera).value();
    camera_description.near_plane = 0.05F;
    if (!fixture.scene->set_perspective_camera(fixture.camera, camera_description) ||
        fixture.scene->perspective_camera(fixture.camera).value().near_plane != 0.05F) {
        return 8;
    }
    return 0;
}
...
[[nodiscard]] int verify_snapshot_items(const elf3d::SceneHierarchySnapshot& snapshot,
    ...
    for (std::size_t index = 0; index < snapshot.size(); ++index) {
        const elf3d::Result<elf3d::SceneHierarchyItem> item = snapshot.item(index);
        const elf3d::Result<std::string_view> name = snapshot.name(index);
        if (!item || !name) {
            return 92;
        }
    ...
    if (!found_public_model || snapshot.item(snapshot.size()).error().code() !=
...
[[nodiscard]] int verify_scene_visibility(PublicSceneFixture& fixture) {
    ...
    if (!fixture.scene->set_entity_visible(fixture.model, false) ||
        ...
        !fixture.scene->set_entity_visible(fixture.model, false) ||
...
int main() {
    ...
    const int visibility = verify_scene_visibility(fixture);
    if (visibility != 0) {
        return visibility;
    }
    ...
    return verify_late_scene_lifetime(cpu_configuration);
}
