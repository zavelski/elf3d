...
int main() {
    ...
    if (!scene->set_local_matrix(model.value(), matrix) ||
        ...
        !scene->world_bounds().has_value() || scene->statistics().model_entities != 1) {
        return 9;
    }
    ...
    if (!scene->set_entity_visible(model.value(), false) ||
        ...
        scene->visible_bounds().has_value() || !scene->set_entity_visible(model.value(), false) ||
        scene->visibility_revision() == visibility_revision || !scene->show_all_entities() ||
        !scene->visible_bounds().has_value()) {
        return 94;
    }
