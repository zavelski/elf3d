...

    {
        ...
        if (!engine_result) {
            if (engine_result.error().code() == elf3d::ErrorCode::unsupported_graphics_version ||
                ...
                std::cout << "Skipping embedded-model quick test: "
        ...
        const elf3d::Result<void> render_result = viewport->render(*loaded.scene, camera.value());
        ...
        if (!render_result || statistics.draw_calls != 2 || statistics.triangles != 4 ||
            statistics.vertices != 8 || statistics.indices != 12 ||
            statistics.texture_bindings != 3 || statistics.gpu_texture_uploads != 3 ||
            statistics.unique_gpu_textures != 3) {
