...
struct ViewerScene {
    ...
    std::optional<elf3d::Bounds3> source_bounds;
...
[[nodiscard]] NavigationCursorSample navigation_cursor_sample(GLFWwindow *window,
                                                              ViewerState &state,
...
viewport_input_from_imgui(GLFWwindow *window, ViewerState &state, ImVec2 item_min,
                          ...
                          ImVec2 item_size, elf3d::Extent2D render_extent) noexcept {
    const ImGuiIO &io = ImGui::GetIO();
    const float x_scale =
    ...
    const bool tracking_enabled =
        item_hovered || item_focused || pointer_captured || left_button_down ||
    ...
    input.pointer_position_pixels = {
        (cursor_sample.position.x - item_min.x) * x_scale,
        (cursor_sample.position.y - item_min.y) * y_scale,
    };
...
void build_3d_view(GLFWwindow *window, ImGuiID dockspace_id, ViewerState &state,
                   elf3d::Engine &engine, elf3d::Viewport &engine_viewport,
                   ViewerScene &active_scene) {
    ...
    if (is_visible) {
        ...
        if (!resize_result) {
            ...
            const bool pointer_captured = state.application_focused &&
                                          !navigation_blocked_by_modal() &&
                                          (state.cursor_captured ||
                                           (snapshot.has_value() &&
                                            snapshot->is_pointer_captured));
            const elf3d::ViewportInput input =
                viewport_input_from_imgui(window, state, image_min, image_hovered, input_focused,
                                          pointer_captured, area_size, state.view_dimensions);
            ...
            sync_navigation_cursor(window, state, engine_viewport, true);
...
void build_selection_panel(ImGuiID dockspace_id, ViewerState &state, const ViewerScene &scene,
    ...
    if (begin_panel_window("Selection", &state.show_selection_panel, state.panel_title_font)) {
        ...
        const elf3d::Result<elf3d::PickingStatistics> picking_result =
            engine_viewport.picking_statistics();
        if (!picking_result) {
            set_viewport_error(state, picking_result.error());
            ImGui::TextUnformatted("Picking statistics unavailable");
        } else {
        const elf3d::PickingStatistics &picking = picking_result.value();
...
void build_status_bar(const ViewerState &state, const elf3d::Engine &engine,
    ...
    if (ImGui::Begin("##Elf3DStatusBar", nullptr, flags)) {
        ...
        const std::string clipping = clipping_status(
            engine_viewport.clipping_snapshot(),
            visible_bounds && visible_bounds.value().has_value());
...
int run_viewer(int argument_count, char **arguments) {
    ...
    elf3d::Result<std::unique_ptr<elf3d::imgui::Context>> imgui_result =
        elf3d::imgui::Context::create(window.get(), glsl_version, imgui_options);
    if (!imgui_result) {
        std::cerr << imgui_result.error().message() << '\n';
        return 1;
    }
    ...
    while (glfwWindowShouldClose(window.get()) == GLFW_FALSE) {
        ...
        build_3d_view(window.get(), dockspace_id, state, *engine, *engine_viewport, active_scene);
